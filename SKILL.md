...
steps:
  ...
  - id: copy
    ...
    gate: test -s "$OUT"               # gate is the floor; judge never replaces it
...
- **Durable local run boundary:** every new run freezes exact `workflow.yaml`, workflow/input SHA-256, atomic manifest/state, and a contiguous trace under a one-writer lock. `piw resume WORKFLOW RUN` verifies that boundary and runs only unfinished nodes. Workflow drift needs audited `--force-drift`; that flag executes the reviewed current source and retains the original snapshot as evidence. Input drift is never forceable. This is local-filesystem crash recovery, not exactly-once external effects.
...
### Corpus contract for `piw batch`

`--inputs` accepts either form:

- **JSONL**, one record per line: `{"content": "...", "id": "optional-name"}`.
  `content` is required and becomes that item's immutable input.
- **A directory**, in which case each file is one item and its filename stem
  becomes the item `id`.

Two things that are easy to get wrong: the per-item run copies the input to
`input.txt`, so the original filename does **not** survive into the run — use
the `id` in `outputs.jsonl` if the report must name its sources. And batch is
strictly per-item: it never aggregates. A request like "process each file, then
write one combined report" is two workflows — the per-item graph under `batch`,
and a second graph that consumes `outputs.jsonl`.

`--resume <dir>` re-checks the frozen contract, so re-pass the same `--inputs`,
`--require-all`, and `--output-step` flags or it refuses.

4. **Pick the model with data:** `piw eval steps.yaml --inputs corpus.jsonl --models luna,sol,...` — swaps only the top-level default (judges stay fixed), `--no-cache`, paired inputs → `eval-report.md`: pass rate, QA rate, judge scores, tokens, cost, wall per model. Choose on evidence, not vibes. A non-zero exit means at least one item failed; read the report before drawing conclusions.
5. **Open the Studio when a human drives it:** `piw ui steps.yaml --input-file idea.md` — the optional local evidence workspace discovers durable run history, renders the selected run's frozen graph, and synchronizes visible node status, committed trace, provenance, output/stderr/attempts, immutable input, and canonical launch. Interrupted durable runs expose a copy-only resume command; legacy/degraded bundles remain visibly bounded and read-only. `--output <step-id>` picks the result artifact; the UI never owns workflow semantics.
...
7. **Use the durable optimizer for multi-candidate work:** first require `piw version --json` and `piw doctor --json` to pass without integrity drift, and discover valid model ids with `piw models` (or pre-flight one with `piw models --check <id>`). Generate a valid starter contract instead of hand-writing one: put a development corpus and a private holdout (both JSONL) next to the workflow, then run `piw optimize scaffold WORKFLOW --inputs DEV.jsonl --holdout PRIVATE.jsonl --json`. It writes `optimization-contract.json` plus a frozen default evaluator beside `steps.yaml`, derives one mutable mechanism per model step (prompt/model/thinking), records only the holdout digest, and validates against `schemas/optimization-contract.schema.json`. Read the contract and tune budgets, `minimum_gain`, promotion thresholds, and `boundaries.mutable` before spending. Then run `piw optimize init WORKFLOW --contract CONTRACT --out EXP --json` and `piw optimize baseline EXP --json`. Propose one candidate file from the current incumbent, changing exactly one `boundaries.mutable` JSON Pointer, and submit it with `piw optimize candidate EXP --file FILE --parent INCUMBENT --mechanism ID --hypothesis TEXT --json`. Code—not the agent—owns batch execution, gates, uncertainty-adjusted keep/revert, exact rollback, budgets, plateau, append-only evidence, and crash recovery. End search with `piw optimize stop`, then permit `piw optimize promote --holdout-file PRIVATE.jsonl` once; inspect `piw optimize receipt`. A promotion receipt is evidence only and never authorizes commit, push, merge, deployment, or production writes. The controller does not invoke those effects, but workflow nodes keep normal user authority; require an external sandbox for strict effect/network isolation. See `docs/optimization.md`.
8. **Automate only after proof:** `piw schedule <workflow> --interval-minutes N` or `--daily HH:MM`, inspected with `piw automations`. Two hard constraints: scheduling delegates to an external adapter that is **not bundled with this repo**, so these commands are inert on a stock install; and a scheduled run takes no input, so only schedule a workflow whose `input` is not `required`. Never present a schedule as working without confirming both.

Everything above is the whole product. If a request cannot be expressed as
nodes, gates, and QA in `steps.yaml`, say so plainly rather than inventing a
heavier path.
