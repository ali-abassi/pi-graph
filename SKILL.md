...
4. **Pick the model with data:** `piw eval steps.yaml --inputs corpus.jsonl --models luna,sol,...` — swaps only the top-level default (judges stay fixed), disables cache, and runs paired inputs. Read per-item improvements/regressions, Wilson pass intervals, median/p95 cost and latency, QA/judge evidence, and the recommendation. `inconclusive` means gather the requested additional items; never promote a model from an aggregate mean or a corpus smaller than five. A non-zero exit means at least one item failed.
...
7. **Use the durable optimizer for multi-candidate work:** first require `piw version --json` and `piw doctor --json` to pass without integrity drift, and discover valid model ids with `piw models --check <id>`. Generate a starter contract with `piw optimize scaffold WORKFLOW --inputs DEV.jsonl --holdout PRIVATE.jsonl --json`; read and tune its budgets, `minimum_gain`, promotion thresholds, and `boundaries.mutable`. Run `piw optimize init WORKFLOW --contract CONTRACT --out EXP --json`, then `piw optimize baseline EXP --json`. For each candidate: inspect `piw optimize mechanisms EXP --json`; export exact incumbent bytes with `piw optimize checkout EXP --out CANDIDATE.yaml --json`; edit one mechanism; run the free `piw optimize diff EXP --file CANDIDATE.yaml --parent INCUMBENT --json`; submit with `piw optimize submit EXP --file CANDIDATE.yaml --parent INCUMBENT --hypothesis TEXT --json`. Use each response's `next_actions` argv instead of reconstructing commands. Code—not the agent—owns evaluation, keep/revert, exact rollback, budgets, plateau, evidence, and crash recovery. End search with `stop`, permit `promote --holdout-file PRIVATE.jsonl` once, then inspect `receipt`. A receipt is evidence only and never authorizes commit, push, merge, deployment, or production writes; use an external sandbox for strict effect/network isolation. See `docs/optimization.md`.
